		...
		module Action
			...
			class ConfigClone < ProxmoxAction
				...
				def call env
					...
					# Ludus templates já têm rede/bridge configurada (vmbr1000) e não usamos forwarded ports.
					# Se isto estiver ativo, saltamos a lógica de "node ip" que aqui está a dar 400.
					if config.disable_adjust_forwarded_port
						env[:ui].detail "Skipping node ip / forwarded port adjustments (disable_adjust_forwarded_port=true)"
						return @app.call(env)
					end
