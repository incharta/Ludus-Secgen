	...
	module Proxmox
		class Connection
			...
			def initialize(api_url, opts = {})
				ep = api_url.to_s.strip
				# Ensure scheme is present; Proxmox API is normally HTTPS on 8006
				ep = "https://#{ep}" unless ep.start_with?("http://") || ep.start_with?("https://")
				@api_url = ep
			...
			def clone_vm(node: required('node'), vm_type: required('node'), params: required('params'))
				...
				File.open(lock_file_path, File::CREAT) do |file|
					...
					while retries > 0
						...
						wait_response = wait_for_completion task_response: response,
															timeout_message: 'vagrant_proxmox.errors.create_vm_timeout'
			...
			def headers
				...
				ticket.nil? ? base : base.merge(CSRFPreventionToken: csrf_token, cookies: { PVEAuthCookie: ticket })
			end
